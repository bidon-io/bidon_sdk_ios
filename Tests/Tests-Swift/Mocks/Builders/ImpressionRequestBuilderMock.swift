//
//  ImpressionRequestBuilderMock.swift
//  Tests-Swift
//
...
final class ImpressionRequestBuilderMock: BaseRequestBuilder, ImpressionRequestBuilder {
    ...
    var invokedWithPath = false
    var invokedWithPathCount = 0
    var invokedWithPathParameters: (path: Route, Void)?
    var invokedWithPathParametersList = [(path: Route, Void)]()

    func withPath(_ path: Route) -> Self {
        invokedWithPath = true
        invokedWithPathCount += 1
        invokedWithPathParameters = (path, ())
        invokedWithPathParametersList.append((path, ()))
        return self
    }
